                            ...
                            if chip.is_double():
                                for new_chip in permutation(self.chips):
                                    if open_number in new_chip and chip != new_chip:
                                        ...
                                        break
                            return [ChipNodeList([cn]), row.get_index()]

    def play_forced(self, board: Board) -> [ChipNodeList, int]:
        for number in permutation(board.get_forced_numbers()):
            for chip in permutation(self.chips):
                if number in chip:
                    logging.info("%s plays :%s" % (self.name, chip))
                    cn = ChipNode(chip, number)
                    return [ChipNodeList([cn]), board.get_forced_row_index()]
